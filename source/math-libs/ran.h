...
struct Ran
{
    ...
    inline Doub gaussian() { return sqrt (-2.0*log (doub ())) * cos (2.0 * M_PI *doub ()); }
    inline Uint int32() { return (Uint)int64(); }

    Doub norm(Doub mu, Doub sig)
    {
        return mu+sig*gaussian();
    }

    Doub logNorm(Doub mean,Doub std)
    {
        Doub cv = std/mean;
        Doub mu = log(mean)-0.5*log(1+cv*cv);
        Doub sig = sqrt( log(cv*cv+1) );
