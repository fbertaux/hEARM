...
struct Odeint
{
    ...
    Odeint(int numVariables, const Doub atol,const Doub rtol,const Doub h1, const Doub hminn,Output &outt,typename Stepper::Dtype &derivss);
    void integrate(Doub deltaT);

    void displayStatus();
};

template<class Stepper>
Odeint<Stepper>::Odeint(int numVariables, const Doub atol, const Doub rtol, const Doub h1, const Doub hminn,
                        Output &outt,typename Stepper::Dtype &derivss) : nvar(numVariables),
y(nvar),dydx(nvar),x(0.),nok(0),nbad(0),
x1(0.),x2(0.),hmin(hminn),dense(outt.dense),out(outt),derivs(derivss),
...
template<class Stepper>
void Odeint<Stepper>::integrate(Doub deltaT)
{
    x=0; x1=0; x2 = deltaT;
