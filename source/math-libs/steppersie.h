#ifndef DEF_STEPPERSIE
#define DEF_STEPPERSIE
...
void StepperSie<D>::prepare_dense(const Doub h,VecDoub_I &ysav,VecDoub_I &scale,
                                  const Int k,Doub &error)
{
    if (h<0) { cout << "I don't like negative step. And I want to remove a warn." << endl; exit(2); }
    if (error<0) { cout << "I don't like negative error. And I want to remove a warn." << endl; exit(3); }
    if (scale[0]<0) { cout << "I don't like negative scale. And I want to remove a warn." << endl; exit(4); }
